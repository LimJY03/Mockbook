...
public class AccountSetUp extends TraceBack {
    ...
    public void setGender() {
        ...
        // MainProgram.db.columnAdder("Gender", "varchar(255)"); // checks if there is a
        // table for the Gender if not make one.
        MainProgram.db.updateTable("Gender", this.gender, this.username); // update the table if there is one with the
                                                                          // new gender for the username
    ...
    public boolean birthValidator(String birthday, String birthmonth, String birthyear) {
        try { // check if the month/day is lower than 10 so its going to be single digit we
              // have to add 0 next to it
            ...
            if (Integer.parseInt(birthyear) < 1900  || Integer.parseInt(birthyear) > 2023) {
                System.out.println("Please Provide an Appropiate Year!");
                return false;
            }
            ...
            return true;
        } catch (DateTimeParseException e) // throws an exception if the date is wrong " typed 13 for month for examople
                                           // or used a date we didn't reach yet
        {
    ...
    public void setJob() {
        // MainProgram.db.columnAdder("Job", "varchar(255)"); // checks if there is a
        // Column for Job if not make one.
        System.out.print("Input Your Job: ");
        String getString = MainProgram.sc.nextLine();

        MainProgram.db.updateTable("Job", getString, this.username); // update the table if there is one with the new
                                                                     // job for the username
        this.job = getString;
    ...
    public void setHobbies(String hobbies) {
        // MainProgram.db.columnAdder("Hobbies", "varchar(500)"); // checks if there is
        // a Column for Hobbies if not make one.
        MainProgram.db.updateTable("Hobbies", hobbies, this.username); // update the table if there is one with the new
                                                                       // hobbies for the username
    ...
    public void setAddress() {
        ...
        System.out.println("---------------------------------------");
        // MainProgram.db.columnAdder("Address", "varchar(255)"); // checks if there is
        // a Column for the Address if not make one.
        MainProgram.db.updateTable("Address", in, this.username); // update the table if there is one with the new
