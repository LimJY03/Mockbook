...
public class AddNewFriend extends TraceBack {
	...
	public static ArrayList<String> sentFriendRequestList(ArrayList<String> sentFriendRequests, ArrayList<String> friendRecommendation, int index, String username) {
	...
	private static void updateDBAfterSent(String username, String targetUsername, ArrayList<String> newSentRequestList) {

		try {

			PreparedStatement stmt = MainProgram.connection.prepareStatement("SELECT ReceivedFriendRequest FROM User WHERE Username = '" + targetUsername + "'");
			...
			if (rs.next()) {
				...
				newReceivedList = (newReceivedList == null) ? username + "," : newReceivedList + username + ",";
			}

			stmt = MainProgram.connection.prepareStatement("UPDATE User SET ReceivedFriendRequest = '" + newReceivedList + "' WHERE Username = '" + targetUsername + "'");
			stmt.executeUpdate();
	...
	private static void updateDBAfterReceived(String username, String targetUsername, ArrayList<String> newReceivedRequestList, boolean accept) {

		try {

			PreparedStatement stmt = MainProgram.connection.prepareStatement("SELECT SentFriendRequest FROM User WHERE Username = '" + targetUsername + "'");
			ResultSet rs = stmt.executeQuery();
			...
			if (accept) {
				...
				myFriendList = (myFriendList == null) ? targetUsername + "," : myFriendList + targetUsername + ",";
				...
				targetFriendList = (targetFriendList == null) ? username + "," : targetFriendList + username + ",";
	...
	public static ArrayList<String> processReceiveFriendRequest(RegularUser me, ArrayList<String> receivedFriendRequestList, int acceptOrDeclineOrView, int option) {
	...
	private static ArrayList<ArrayList<String>> getSentReceivedRequest(RegularUser me) {
		...
		try {
			...
			PreparedStatement stmt = MainProgram.connection
					.prepareStatement("SELECT SentFriendRequest, ReceivedFriendRequest FROM User WHERE Username = '"
							+ me.getUsername() + "'");
			ResultSet rs = stmt.executeQuery();
			...
			if (rs.next()) {
				...
				if (sentRequestHolder != null) {
					...
					for (String sent : sentRequestArr)
						sentRequest.add(sent);
				}
				if (receivedRequestHolder != null) {
					receivedRequestArr = receivedRequestHolder.split(",");
					for (String sent : receivedRequestArr)
						receivedRequest.add(sent);
				}
